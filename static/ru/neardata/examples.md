...
Shell-примеры ниже выполняются на NEAR Data-хостах и передают `FASTNEAR_API_KEY` из вашего shell как query-параметр `apiKey`, поэтому сначала задайте её. Также поддерживается bearer-аутентификация через `Authorization: Bearer ${FASTNEAR_API_KEY}`, если вашему клиенту удобнее передавать ключ в заголовке.
...
- Используйте [Transactions API](https://docs.fastnear.com/ru/tx), когда у вас уже есть `tx_hash` и нужна человекочитаемая история транзакции.
