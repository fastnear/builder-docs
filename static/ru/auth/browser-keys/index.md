**Источник:** [https://docs.fastnear.com/ru/auth/browser-keys](https://docs.fastnear.com/ru/auth/browser-keys)
...
| --- | --- | --- |
| **Серверный ключ** (по умолчанию) | бэкенд, воркер, прокси, менеджер секретов | хранится в секрете; никогда не отправляется клиентам. См. [Аутентификацию и доступ](https://docs.fastnear.com/ru/auth). |
...
:::info Доступность
Ограничение по сайтам — функция платных тарифов. На бесплатном тарифе этот элемент управления отключён — повысьте тариф проекта в [FastNear Dashboard](https://dashboard.fastnear.com), чтобы включить браузерные ключи.
:::
...
1. Откройте проект в [FastNear Dashboard](https://dashboard.fastnear.com) и создайте (или выберите) API-ключ.
...
- Никогда не помещайте в браузер учётные данные, которые должны оставаться секретом. Для всего чувствительного или неограниченного используйте [серверный ключ](https://docs.fastnear.com/ru/auth) за своим бэкендом.
...
- [Аутентификация и доступ](https://docs.fastnear.com/ru/auth) — серверный ключ по умолчанию и выбор способа передачи
- [Аутентификация для агентов](https://docs.fastnear.com/ru/agents/auth) — режим выполнения для агентов и автоматизаций
