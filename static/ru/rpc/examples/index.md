...
Shell-примеры ниже выполняются на RPC-хостах и передают `FASTNEAR_API_KEY` из вашего shell как query-параметр `apiKey`, поэтому сначала задайте её. Также поддерживается bearer-аутентификация через `Authorization: Bearer ${FASTNEAR_API_KEY}`, если вашему клиенту удобнее передавать ключ в заголовке.
