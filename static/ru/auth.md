...
Один API-ключ FastNear работает и для [RPC](https://docs.fastnear.com/ru/rpc), и для [API-эндпоинтов](https://docs.fastnear.com/ru/api). Держите модель аутентификации простой: используйте один и тот же ключ везде и передавайте его либо через заголовок `Authorization: Bearer`, либо как URL-параметр `?apiKey=`.
...
Получите ключ в [FastNear Dashboard](https://dashboard.fastnear.com) и отправляйте его в каждом запросе одним из способов ниже.
