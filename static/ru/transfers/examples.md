...
Эти shell-примеры выполняются на Transfers и Transactions endpoint-ах и передают `FASTNEAR_API_KEY` из окружения как query-параметр `apiKey`, поэтому сначала задайте её. Также поддерживается bearer-аутентификация через `Authorization: Bearer ${FASTNEAR_API_KEY}`, если вашему клиенту удобнее передавать ключ в заголовке.
