---
title: "API переводов"
description: "История переводов NEAR и FT-токенов по аккаунтам для продуктовых лент и инструментов расследования."
sidebar_position: 1
...
OpenAPI: [JSON](/openapi/transfers.json) · [YAML](/openapi/transfers.yaml) · [все спецификации](/openapi)
...
Используйте [Транзакции API](/tx) для более широкой истории исполнения и [FastNear API](/api) для ответов в стиле состояния аккаунта.
...
1. Начните с [Запроса переводов](/transfers/query), используя самый узкий набор фильтров, который всё ещё отвечает на вопрос.
...
Тогда расширяйтесь на [Транзакции API](/tx) или [FastNear API](/api), а не перегружайте представление переводов.
...
Используйте [Transfers API Examples](/transfers/examples) для практических примеров: узкие поиски переводов, пагинация через `resume_token` и переход к более широкому расследованию транзакций.

## Устранение неполадок

### Нужны полные метаданные транзакции

Переходите на [Транзакции API](/tx), если одной истории переводов недостаточно.
