...
<div className="fastnear-doc-card-grid">
  ...
  <article className="fastnear-doc-card">
    ...
    <span>Посмотрите, как API-ключи FastNear работают в интерфейсе документации и как перейти от демо-сценариев к безопасному серверному использованию.</span>
    <span className="fastnear-doc-card__bestfor-label">Лучше всего подходит для:</span>
    <ul className="fastnear-doc-card__bestfor-list">
      <li><Link to="/auth">обзора аутентификации</Link></li>
      <li><Link to="/auth/browser-demo">браузерного демо-сценария</Link></li>
      <li><Link to="/auth/backend">аутентификации продакшен-бэкенда</Link></li>
    </ul>
  </article>
  <article className="fastnear-doc-card">
...
- [Аутентификация и доступ](/auth): разберитесь в различии между встроенным демо-сценарием документации и аутентификацией продакшен-бэкенда.
...
- Начните со страницы [Как выбрать подходящую поверхность](/agents/choosing-surfaces), чтобы сопоставить задачу с правильной поверхностью FastNear.
...
- Берите за основу [Плейбуки для агентов](/agents/playbooks), если нужен конкретный шаблон рабочего процесса.
- Используйте [Аутентификацию и доступ](/auth), чтобы хранить учётные данные агентов на серверной стороне.
