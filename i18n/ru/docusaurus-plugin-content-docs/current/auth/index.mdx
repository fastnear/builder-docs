...
Один API-ключ FastNear работает и для [RPC](/rpc), и для [API-эндпоинтов](/api). Держите модель аутентификации простой: используйте один и тот же ключ везде и передавайте его либо через заголовок `Authorization: Bearer`, либо как URL-параметр `?apiKey=`.
...
Получите ключ в <Link href="https://dashboard.fastnear.com">FastNear Dashboard<IconExternalLink /></Link> и отправляйте его в каждом запросе одним из способов ниже.
