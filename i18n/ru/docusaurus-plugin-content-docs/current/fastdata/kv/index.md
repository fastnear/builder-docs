...
Используйте [примеры KV FastData](/fastdata/kv/examples) для простых пошаговых сценариев: поиска по точному ключу, истории точного ключа, анализа по `predecessor_id` и привязки к транзакции.
...
Тогда расширяйтесь на [Просмотр состояния контракта](/rpc/contract/view-state) в [Справочнике RPC](/rpc) или на собственный read-метод контракта. Не предполагайте, что один ключ FastData напрямую соответствует одному raw-ключу из `view_state`.
