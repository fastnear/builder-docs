...
- **`src/data/generatedFastnearPageModels.json`** — Vendored page-model registry generated in `mike-docs`. This is the source of truth for native docs rendering here.
  ...
  - `url` is the endpoint "Send request" hits. For the sixteen operations whose *examples* pin a historical record, that is `archival-rpc`, because the standard RPC drops those records after roughly 29 hours. That is a property of the example, not of the method — every one of those methods works on the standard RPC for recent data. When `archivalReason` is present, `FastnearDirectOperation` renders it as a note under the endpoint and `scripts/generate-ai-surfaces.js` appends it to the `## Networks` lines, so agents reading the AI surface get the reason rather than inferring a hard requirement. Declared in `ARCHIVAL_EXAMPLES` in mike-docs `scripts/rpc-example-config.js`; never patch it here.
- **`static/openapi/`** — Published OpenAPI documents, vendored from mike-docs `shared/openapi/` (generated by `scripts/generate-openapi-artifacts.js` there, never edited here): `index.json`, one bundled `<family>.json`/`.yaml` per spec family (rpc, fastnear, transactions, transfers, kv-fastdata, neardata), and one self-contained `<family>/<operationId>.json`/`.yaml` per operation. Page models carry an `openapi` block pointing at them; the operation UI, page heads (`rel="service-desc"` / `rel="describedby"`), AI surfaces, `llms.txt`, JSON-LD, and the `/openapi` hub page all link to them.
