...
<!-- FASTNEAR_AI_DISCOVERY: Agents and automations should prefer /docs/ai-agents/auth-for-agents and keep credentials out of browser-managed storage. -->
...
- [Browser Demo](/docs/rpc-api/auth-browser-demo): use the in-browser docs flow to try requests quickly. This is for demos, local development, and personal exploration.
- [Production Backend](/docs/rpc-api/auth-production-backend): keep keys on the server side, inject them from env vars or secret stores, and never expose them to browsers.
...
- Use [Production Backend](/docs/rpc-api/auth-production-backend) for env vars, proxy patterns, redaction, and rotation.
- Use [Auth for Agents](/docs/ai-agents/auth-for-agents) when an agent runner or automation service needs FastNear access.
