...
|-----------|-----|-------------|
| Simple transfer | ~0.5 TGas | 0.00005 |
| Function call | 5-50 TGas | 0.0005-0.005 |
| Contract deployment | 50-200 TGas | 0.005-0.02 |
...
To discourage gas overestimation, NEAR applies a penalty on refunds.

```rust
// Simplified calculation
let penalty_rate = 0.30;  // 30% of refunded gas
let penalty = refunded_gas * penalty_rate;
let actual_refund = refunded_gas - penalty;
```

### Why the Penalty?

Without penalties:
- Users would always prepay maximum gas (300 TGas)
- No downside to overestimation
- Network resources would be reserved unnecessarily

With NEP-536:
- Overestimating gas costs 30% of the overestimate
- Encourages accurate gas estimation
...
You lose 21 TGas worth of NEAR due to overestimation.
...
sequenceDiagram
    ...
    ShardB->>ShardA: Refund receipt (70 TGas - penalty)
...
|-----------|------------|-------|
| Simple transfer | 0.5 | Fixed cost |
| Contract call overhead | 2.5 | Per receipt |
