...
slug: /transaction-flow/gas-economics
sidebar_position: 6
keywords:
  - NEAR
  - gas
...
For docs users, the refund path is simpler than the runtime code:

1. gas is prepaid up front
2. execution burns part of that gas
3. unused gas becomes a refund amount
4. the refund comes back as a later receipt
...
When blocks are busier, protocol gas price rises. When blocks are quieter, it falls. For docs users, the important part is that this adjustment is protocol-driven, not a fee-bidding market.
...
Refunds often land in later blocks, especially when cross-shard receipts are involved. Do not expect the account balance view to reflect them immediately at the moment the first transaction outcome appears.
