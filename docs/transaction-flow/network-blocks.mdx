...
slug: /transaction-flow/network-blocks
sidebar_position: 7
keywords:
  - NEAR
  - network
...
For docs users, the exact scheduler is less important than three user-visible consequences:

- inclusion is fair rather than fee-bid driven
- transactions for the same account/key still respect nonce order
- full chunks delay inclusion instead of reordering for higher bids
...
### Production Flow
From a docs perspective, chunk production matters only because it determines when a transaction or receipt first becomes visible in block-scoped data. You usually do not need the producer handshake; you need the resulting block height, shard placement, and receipt timing.
...
For docs users, keep the ordering rule simpler than the runtime implementation:

1. local receipts from this block's transactions
2. delayed receipts from older backlog
3. incoming cross-shard receipts
4. yield timeouts

The exact phase boundary matters only when you are explaining why one receipt became visible before another.
