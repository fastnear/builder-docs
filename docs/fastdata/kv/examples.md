...
The first lookup answers “what do we have right now?” Reusing the exact `key` in `POST /v0/history` answers “how did it get here?” If that result is too broad, narrow back down with [GET History by Exact Key](/fastdata/kv/get-history-key).
...
- The user needs the exact current chain state rather than the index. Move to [View State](/rpc/contract/view-state).
