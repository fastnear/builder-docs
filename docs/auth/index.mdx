...
Get a key in <Link href="https://dashboard.fastnear.com">FastNear Dashboard<IconExternalLink /></Link>.
...
- Set `FASTNEAR_API_KEY` in your shell, runtime, or secret manager.
...
- Building a browser or frontend app? Ship a [browser key](/auth/browser-keys) restricted to your domains, not a plain server key.
...
### The request works in one context but not another

That usually means the key is not being sent in the second context. Add the same FastNear key you use elsewhere.
...
### I am building a browser or frontend app

Do not ship a server key in your frontend. Use a [browser key](/auth/browser-keys) restricted to your site's domains, with a per-key rate cap, so a copied key is hard to reuse and cannot drain your quota.
