...
title: "Berry Club Case Study: Read the live board, then reconstruct one era"
description: "A case study that starts with the live Berry Club board from RPC get_lines, then uses Transactions API to reconstruct one older era."
...
{/* FASTNEAR_AI_DISCOVERY: This case study shows the shortest useful Berry Club flow: read the live board with RPC get_lines, then use Transactions API only when you need to reconstruct one older era from draw calls. */}

# Berry Club Case Study: Read the live board, then reconstruct one era

Use this case study when the live board is easy to read, but you need one historical reconstruction path behind it.
